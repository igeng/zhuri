"""Entry B — interactive REPL + live dashboard (§4.2, primary UX).
...
dashboard. Slash-commands are control-only (they never violate B1 on the run
path). The REPL itself is the only sanctioned interactive surface besides
``config`` and the one-time spec confirmation.
"""
